...
public class Board : MonoBehaviour
{
    ...
    {
        if (UnityEngine.Random.Range(0f, 100f) < bombChance)
        {
            gemToSpawn = bomb;
        }
    ...
    public void ShuffleBoard()
    {
        if (currentState != BoardState.wait)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    int gemtoUse = UnityEngine.Random.Range(0, gemsFromBoard.Count);
                    int iterations = 0;
                    while (MatchesAt(new Vector2Int(x, y), gemsFromBoard[gemtoUse]) && iterations < 100 && gemsFromBoard.Count > 1)
                    {
                        gemtoUse = UnityEngine.Random.Range(0, gemsFromBoard.Count);
                        iterations++;
                    }
